...
class LearnerGoal(Base):
    ...
    status = Column(String, default="active", index=True) # active/completed/changed
...
class GoalSkillRequirement(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    goal_id = Column(Integer, ForeignKey("learner_goals.id"), index=True)
...
class MilestoneItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    milestone_id = Column(Integer, ForeignKey("roadmap_milestones.id"), index=True)
